...
Think of HTML, String, JS, JSX, and CSS land. {} signal to JSX that we are now
in JS land.

When we introduce `{}` we are embedding a javascript expression. Whatever is
between the {} is JS stuff. So we can use this to tell JSX to return the value
of a variable here. We can do more complex evaluation as well, such as a
ternaries or function calls. But, we cannot write JS statements, only
expressions.

We can only make expressions when interpolating because they actually evaluate
to something. We cannot introduce JS statements i i.e. code blocks or imperative
logic because the `{}` signals a JS expression in JSX.

Which means, we cannot nest `{}`within another `{}` because they signal logic,
but we must evaluate something, so we can still do ternaries, or a function call
where we work out the logic earlier.
