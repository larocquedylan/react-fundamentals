...
Try to give users control of the interface in a way they will understand and
abstract this to fit your application. For intsance, instead of asking them to
sade className=box--size, introduce a prop called size, which changes the
classname to box--size under the hood.

When defining an expression it must be {}, so remember when defining styling
inline you passing another object, it so it needs to be in object form.

As well, if your a variable, use template literals, so that Js knows we are
passing a string and spaces, but also the fill the placeholder with the value
