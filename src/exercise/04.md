...
Custom UI component function need to be capitalized `<Capitalized />` for
compiler to recognize they are custom components. This is key because we are
abstracting `React.createElement` which takes an html element as the first
arugment. So, if don't capitalize and render our function, babel will think the
element type is whatever we called our function, rather than a div.

Use `prop-types` npm package to validate our prop types. Typescript is kind of
the wave, but if you aren't there yet (as I am not), prop-types gives some
validation coverage in the meantime. As well, babel and other tools actually
remove this validation in production because it has a computational cost so will
slow down our ui. We don't have to worry about handling that as it does it for
us when we change from development to production.
