...
Use the console on the webpage to check what objects include i.e. type`React` or
`ReactDOM` and you will see the different methods available for the object.

The difference between React and ReactDOM is that React is what actually allows
us to create React elements, whereas ReactDOM is what appends them to the DOM in
the browser, through JS. They are different because React can support React
Native to work with VR or IOS. So we build it in React and render with the
platform specific render tool, in our case ReactDOM.

The ReactDOM is a virtual DOM which is what is used in the browser. It doesn't
rerender everytime, we have to tell it when to.

syntax: `const element = React.createElement('elementtype', {properties passed},
child1, child2, ...childN)

what is the same as JS?

- We still create the `<div id="root" />` that we use the
  `document.getElementById('root')` to append to the DOM.
- We basically `createElement`the same way

What is different?

- We use npm or unpkg the React Library to create the global variable `React` to
  be able to use `.createElement()` method.
- We do the same for `ReactDOM` to be able to render it to the DOM
